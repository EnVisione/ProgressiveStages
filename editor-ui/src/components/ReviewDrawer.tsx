...
export function ReviewDrawer() {
  ...
  return <div className="review-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) closeReview(); }}><section className="review-drawer" role="dialog" aria-modal="true" aria-label="Review and apply stage changes">
    <header><div><span className="eyebrow">Final operator review</span><h2>Review and apply</h2><p>Nothing reaches the live server until this complete validation succeeds and you confirm it.</p></div><Button tone="quiet" icon="close" aria-label="Close review" onClick={closeReview}/></header>
