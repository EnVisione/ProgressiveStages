...
export function RegistryPage() {
  ...
  return <div className="registry-page"><header className="page-heading"><div><span className="eyebrow">Live server content</span><h1>Registry explorer</h1><p>Search installed mods, identifiers, names, and tags before adding them to a stage.</p></div><Badge tone="gold">Catalog revision {boot?.catalog.revision || 0}</Badge></header>
    <section className="registry-toolbar"><Field label="Content type"><select value={catalogId} onChange={event => { setCatalogId(event.target.value); setMod(""); }}>{boot?.catalog.ids.map(id => <option key={id} value={id}>{id.replace(/^progressivestages:/, "")}</option>)}</select></Field><Field label="Selector mode"><select value={mode} onChange={event => setMode(event.target.value)}><option value="id">Exact identifier</option><option value="mod">Whole mod</option><option value="tag">Tag</option><option value="name">Name match</option></select></Field><Field label="Search"><div className="input-icon"><Icon name="search" size={16}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Diamond, minecraft, c:ores"/></div></Field><Field label="Installed mod"><select value={mod} onChange={event => setMod(event.target.value)}><option value="">Every mod</option>{mods.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></Field></section>
