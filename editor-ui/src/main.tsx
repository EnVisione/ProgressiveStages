  ...
  return <>
    ...
    <main class="workspace">
      ...
      <section class="canvas panel">
        ...
        <div id="graphView" class="view hidden"><div class="graph-help"><div><strong>Progression grows upward.</strong><span> Beginner paths start at the bottom. Joined paths connect into one stage above.</span></div><div class="graph-controls"><select id="graphCategory" aria-label="Graph category"><option value="">All categories</option></select><input id="graphSearch" placeholder="Find a stage" aria-label="Find a stage"/><span class="graph-zoom"><button id="graphZoomOut" class="ghost" aria-label="Zoom out">−</button><output id="graphZoomValue">100%</output><button id="graphZoomIn" class="ghost" aria-label="Zoom in">+</button></span><button id="fitGraph" class="ghost">Fit graph</button><button id="autoArrangeGraph" class="ghost">Arrange paths upward</button></div></div><div id="graphStatus" class="graph-status"></div><div id="graphViewport" class="graph-viewport"><div id="graph" class="graph"></div></div></div>
