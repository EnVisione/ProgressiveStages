  ...
  return <>
    ...
    <main class="workspace">
      ...
      <section class="canvas panel">
        ...
        <div id="graphView" class="view hidden"><div class="graph-help"><div><strong>Player progression UI layout.</strong><span> Drag icons to place them. Drag empty space to pan. Scroll to move and hold Control while scrolling to zoom. Connect stages to draw a branch. Select a branch line to remove it.</span></div><div class="graph-controls"><select id="graphCategory" aria-label="Graph category"><option value="">All categories</option></select><input id="graphSearch" placeholder="Find a stage" aria-label="Find a stage"/><span class="graph-zoom"><button id="graphZoomOut" class="ghost" aria-label="Zoom out">−</button><output id="graphZoomValue">100%</output><button id="graphZoomIn" class="ghost" aria-label="Zoom in">+</button></span><button id="fitGraph" class="ghost">Fit graph</button><button id="connectGraphStages" class="ghost">Connect stages</button><button id="cancelGraphConnection" class="ghost hidden">Cancel connection</button><button id="resetGraphLayout" class="ghost">Use automatic layout</button><button id="autoArrangeGraph" class="ghost">Arrange and save</button></div></div><div id="graphStatus" class="graph-status"></div><div id="graphViewport" class="graph-viewport"><div id="graphSizer" class="graph-sizer"><svg id="graph" class="graph" role="group" aria-label="Player progression stage layout"></svg></div></div></div>
